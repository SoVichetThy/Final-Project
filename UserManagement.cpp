...
public:
	UserManager()
	{
		User user1("vichet", "11", "ADMIN");
		User user2("rotha", "22", "NORMAL");
		users[count++] = user1;
		users[count++] = user2;
	}
	...
	void viewUser()
	{
		...
		cout << "User Name " << setw(10) << " " << "User password " << setw(10) << " " << " Type " << endl;
		for (int i = 0; i < count; i++)
		{
			cout << users[i].getUsername() <<"\t" << setw(10) << users[i].getPassword() << setw(10) << " " << users[i].getType() << endl;
		}
	}
